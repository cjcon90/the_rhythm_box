...
def review_product(request, product_slug):
    ...
    already_rated = Rating.objects.filter(
            user_id=request.user, product__slug=product_slug).exists()
    already_reviewed = Review.objects.filter(
            rating__user_id=request.user, rating__product__slug=product_slug).exists()
    ...
    # GET request
        ...
        if already_rated:
            prev_rating = Rating.objects.get(user_id=request.user, product__slug=product_slug)
            context['rating_form'] = ProductRatingForm(initial={'rating':prev_rating.rating})
        else:
        ...
        if already_reviewed:
            ...
            context['review_form'] = ProductReviewForm(initial={'headline':prev_review.headline, 'content':prev_review.content})
        else:
